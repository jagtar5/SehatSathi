...
const LabTestForm = ({ 
  ...
  availablePatients = [] // Add a prop to receive filtered patients list
}) => {
  ...
  // Set patients from props and handle preselected patient
  useEffect(() => {
    // If availablePatients were provided, use them
    if (availablePatients && availablePatients.length > 0) {
      setPatients(availablePatients);
    } else {
      // Otherwise, fetch all patients (fallback)
  ...
  }, [preselectedPatient, availablePatients]);
  ...
  return (
    <form className="modal-form" onSubmit={handleSubmit}>
      ...
      <div className="form-group">
        ...
        {patients.length === 0 ? (
          <p className="no-data-message">No patients available. Only patients with appointments can have lab tests ordered.</p>
        ) : (
          <select
            id="patient_id"
            name="patient_id"
